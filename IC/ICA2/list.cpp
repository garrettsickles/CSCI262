...
void list::add(char c) {
	this->_size < CAPACITY ? this->_data[_size++] = c : 0;
}

void list::insert(char c, int position) {
    ++this->_size;
    if(this->_size < CAPACITY && position >= 0 && position < CAPACITY) {
        for(int i = this->_size; i > position; i--)
...
void list::erase(int position) {
	if(this->_size > 0 && position >= 0 && position < CAPACITY) {
